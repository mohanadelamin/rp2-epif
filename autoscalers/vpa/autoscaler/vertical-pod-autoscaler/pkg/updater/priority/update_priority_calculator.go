...
var (
	...
	podLifetimeUpdateThreshold = flag.Duration("in-recommendation-bounds-eviction-lifetime-threshold", time.Minute*os.Getenv("POD_LIFETIME_MINUTES_BEFORE_VPA_UPDATING"), "Pods that live for at least that long can be evicted even if their request is within the [MinRecommended...MaxRecommended] range")
